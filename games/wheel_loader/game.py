...
class PwmActuator(LinearActuator):
    ...
    :type pin: int
    :param: delta_max: Maximum value of pulsewidth difference
...
class CarGame(Game):
    async def on_init(self):
        ...
        lift controls the lift actuator with 1 pwm signal
        tilt controls the tilt actuator with 1 pwm signal
        throttle_shifting modifies the used speed (pwm signal) of the motor
        steering_shifting modifies the used turning angle (pwm signal) of the servo
        lift_shifting modifies the lift actuator range
        tilt_shifting modifies the tilt actuator range
        """
        self.motor = PwmActuator(self.pi, MOTOR_PIN, 500)
        self.steering = PwmActuator(self.pi, STEERING_PIN, 500)
        self.lift = PwmActuator(self.pi, LIFT_PIN, 500)
        self.tilt = PwmActuator(self.pi, TILT_PIN, 500)

        self.throttle_shifting = ShiftGear(self.motor)
        self.steering_shifting = ShiftGear(self.steering)
        self.lift_shifting = ShiftGear(self.lift)
        self.tilt_shifting = ShiftGear(self.tilt)
        ...
        self.io.register_inputs(
            {
                ...
                "lift": self.lift,
                "tilt": self.tilt,
                "lift_shifting": self.lift_shifting,
                "tilt_shifting": self.tilt_shifting,
            }
        )
